...
def read_table(in_file):
    """
    Reads either CSV or Excel (.xlsx, .xls) into a pandas DataFrame.
...
#! This only works for AA exported data for the ScholarName column
#! Maybe we should make this more general.
def parse_name_column(series):
    """
    Takes a pandas Series of names formatted as 'SURNAME, GIVEN_NAME'
    and returns a list of (surname, given_name) tuples with cleanup.
    """
    results = []
    for val in series.dropna():
        ...
        # Remove trailing middle initial (e.g. "ANDREW A")
        given_name = re.sub(r"\b([A-Z])$", "", given_name).strip()
